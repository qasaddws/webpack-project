    ...
    Paging.prototype = {
        ...
        createDom: function() {
            ...
            for (var i = 1; i <= that.options.totalPages; i++) {
                i != 1 ? olDom += '<li>' + i + '</li>' : olDom += '<li class="sel-page">' + i + '</li>';
            }
            that.options.jump ? jumpDom = '<input type="text" placehlider="1" class="jump-text" id="jumpText"><button type="button" class="jump-button" id="jumpBtn">跳转</button>' : jumpDom = '';
            content = '<button type="button" id="firstPage" class="turnPage first-page">首页</button>' +
                ...
                '<ol id="pageSelect" style="transition:all ' + that.options.slideSpeed + 'ms">' +
                olDom +
                '</ol></div>' +
